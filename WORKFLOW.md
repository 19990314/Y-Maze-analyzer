...
step1a_relabel.py  (optional)
        │
        ▼
step1b_fix_clips.py  (optional — scan & fix mislabeled clips)
        │
        ▼
...
### Step 1b — `step1b_fix_clips.py` *(optional)*

| | |
|---|---|
| **Input** | A `*_clips.csv` file + optionally the source video |
| **Output** | Updated `*_clips.csv` with corrected start/stop frames and labels |
| **Output** | Re-extracted clip `.mp4` files for any fixed rows (old file deleted) |
| **Flags** | Rows where `stop_frame` is within ±100 frames of an adjacent clip's boundary are highlighted in red |
| **Where** | Same folder as the CSV |
...
*_clips.csv  (one per recording session, from Step 1)
    │
    ├──► step1b (optional fix) ──► corrected *_clips.csv
    │
